...
export function registerParseTool(server: McpServer): void {
  server.registerTool(
    'logic_md_parse',
    {
      description: 'Parse a LOGIC.md file or content string into a typed spec object. Returns the parsed spec on success, or parse errors on failure.',
      inputSchema: {
        content: z.string().optional().describe('LOGIC.md content to parse directly'),
    ...
    async ({ content, file_path }) => {
      ...
      if (result.ok) {
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ success: true, spec: result.data }, null, 2) }],
        }
      } else {
        return {
