...
import { compileWorkflow, estimateTokens, parse, validate } from "@logic-md/core";
...
export function registerCompileWorkflowTool(server: McpServer): void {
	server.registerTool(
		...
		{
			description:
				"Compile an entire LOGIC.md spec into a workflow execution plan with DAG ordering, quality gates, and fallback policy.",
			inputSchema: {
				...
				context: z
					...
					.optional()
					.describe("Workflow context for compilation"),
			},
