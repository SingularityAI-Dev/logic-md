...
import { parse, validate, compileStep, estimateTokens } from '@logic-md/core'
...
export function registerCompileStepTool(server: McpServer): void {
  server.registerTool(
    ...
    {
      description: 'Compile a single named step from a LOGIC.md spec into an executable form with system prompt segment, output schema, quality gates, and token estimate.',
      inputSchema: {
        ...
        }).optional().describe('Execution context for compilation'),
      },
