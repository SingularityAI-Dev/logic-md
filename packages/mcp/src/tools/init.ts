  ...
  server.registerTool(
    'logic_md_init',
    {
      ...
      inputSchema: {
        ...
        name: z
          .string()
          .optional()
          .describe('Agent name to substitute in the generated file'),
        description: z
          .string()
          .optional()
    ...
    async ({ template, name, description }) => {
      ...
      let templateContent = readFileSync(
        join(config.templatesDir, `${template}.logic.md`),
        'utf8'
      )

      // 5. Patch frontmatter if name or description provided
      if (name !== undefined || description !== undefined) {
        const frontmatterMatch = templateContent.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/)
        if (frontmatterMatch) {
        ...
        content: [
          {
            type: 'text' as const,
            text: JSON.stringify(
              { content: templateContent, template_used: template },
              null,
              2
            ),
          },
