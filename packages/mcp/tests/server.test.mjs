...
describe('MCP server integration', () => {
  ...
  it('tools/list returns exactly 7 tools', async () => {
    ...
    assert.ok(names.includes('logic_md_compile_workflow'), 'should include logic_md_compile_workflow')
  })
  ...
  describe('logic_md_parse', () => {
    it('returns success:true for valid LOGIC.md content', async () => {
      const client = await createTestClient()
      const result = await client.callTool({ name: 'logic_md_parse', arguments: { content: VALID_LOGIC_MD } })
      assert.ok(!result.isError, `should not be an error, got: ${JSON.stringify(result)}`)
      const parsed = JSON.parse(result.content[0].text)
      assert.strictEqual(parsed.success, true, `parsed.success should be true, got: ${JSON.stringify(parsed)}`)
    })
    ...
    it('returns success:false for invalid (empty) content', async () => {
      ...
      const parsed = JSON.parse(result.content[0].text)
      assert.strictEqual(parsed.success, false, `parsed.success should be false for empty content, got: ${JSON.stringify(parsed)}`)
    })
  })
  ...
  describe('logic_md_validate', () => {
    it('returns valid:true for valid LOGIC.md content', async () => {
      const client = await createTestClient()
      const result = await client.callTool({ name: 'logic_md_validate', arguments: { content: VALID_LOGIC_MD } })
      assert.ok(!result.isError, `should not be an error, got: ${JSON.stringify(result)}`)
      const parsed = JSON.parse(result.content[0].text)
      assert.strictEqual(parsed.valid, true, `parsed.valid should be true, got: ${JSON.stringify(parsed)}`)
    })
  })
  ...
  describe('logic_md_lint', () => {
    it('returns issues array and summary for valid LOGIC.md content', async () => {
      const client = await createTestClient()
      const result = await client.callTool({ name: 'logic_md_lint', arguments: { content: VALID_LOGIC_MD } })
      assert.ok(!result.isError, `should not be an error, got: ${JSON.stringify(result)}`)
      const parsed = JSON.parse(result.content[0].text)
      assert.ok(Array.isArray(parsed.issues), `parsed.issues should be an array, got: ${JSON.stringify(parsed)}`)
      assert.ok(
  ...
  describe('logic_md_list_templates', () => {
    it('returns non-empty templates array', async () => {
      ...
      assert.ok(!result.isError, `should not be an error, got: ${JSON.stringify(result)}`)
      const parsed = JSON.parse(result.content[0].text)
      assert.ok(Array.isArray(parsed.templates), `parsed.templates should be an array, got: ${JSON.stringify(parsed)}`)
      assert.ok(parsed.templates.length > 0, `templates should be non-empty, got length: ${parsed.templates.length}`)
    })
  })
  ...
  describe('logic-md://schema resource', () => {
    it('returns a parseable JSON Schema object', async () => {
      ...
      assert.strictEqual(typeof schema, 'object', `schema should be an object, got: ${typeof schema}`)
      assert.ok(schema !== null, 'schema should not be null')
      // JSON Schema objects typically have 'type' or 'properties'
      const hasSchemaKeys = schema.type !== undefined || schema.properties !== undefined || schema.$schema !== undefined
      assert.ok(hasSchemaKeys, `schema should have expected JSON Schema keys, got keys: ${Object.keys(schema).join(', ')}`)
    })
  })
  ...
  describe('logic-md://spec resource', () => {
    it('returns non-empty markdown containing LOGIC.md', async () => {
      ...
      assert.ok(typeof text === 'string' && text.length > 0, 'spec text should be a non-empty string')
      assert.ok(
  ...
  describe('path traversal prevention', () => {
    it('rejects ../ escape attempt', async () => {
      ...
      const text = result.content[0].text
      assert.ok(
        text.toLowerCase().includes('outside') || text.toLowerCase().includes('not allowed') || text.toLowerCase().includes('working directory'),
    ...
    it('rejects symlink pointing outside cwd', async () => {
      ...
      try {
        ...
        assert.strictEqual(result.isError, true, 'should return isError:true for symlink pointing outside cwd')
      } finally {
