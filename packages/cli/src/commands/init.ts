 ...
 */
function reconstructFile(doc: Record<string, unknown>, body: string): string {
	...
	program
		...
		.action(async (opts: InitOptions) => {
			...
			try {
				...
				const templateChoiceRaw = (await rl.question(`Agent type [1]: `)).trim() || "1";
				...
				const strategy = (STRATEGIES as readonly string[]).includes(strategyRaw)
					...
					: (process.stderr.write(
							`warning: unknown strategy "${strategyRaw}", defaulting to "cot"\n`,
						),
						"cot");
				...
				// 7. Output path
				const outputPath = (await rl.question("Output file [./LOGIC.md]: ")).trim() || "./LOGIC.md";
				...
				if (existsSync(outputPath)) {
					...
					const confirm = (await overwriteRl.question(`Overwrite ${outputPath}? [y/N]: `))
						.trim()
						.toLowerCase();
					overwriteRl.close();
					if (confirm !== "y") {
