...
import { parse, validate, resolve, compileStep, estimateTokens, evaluate, ExpressionError } from '@logic-md/core'
import type { ExecutionContext, ValidationError, LogicSpec, JsonSchemaObject } from '@logic-md/core'
import type { Colors } from '../output/color.js'
import { formatError } from '../output/formatter.js'
import type { StepTestResult, TestResult, ContractCheck, BranchEval } from '../types.js'
...
function checkEdgeContract(
  ...
  for (const key of toKeys) {
    ...
    if (!fromProp) {
      ...
      if (fromType !== toType) {
        failures.push(`type mismatch: ${fromName} outputs ${String(fromType)}, ${toName} expects ${String(toType)}`)
      }
    }
  }
...
export function registerTestCommand(program: Command, colors: Colors): void {
  program
    ...
    .action(async (file: string, opts: { step?: string; input?: string; json?: boolean }) => {
      ...
      process.stdout.write(`DAG Execution Plan: ${colors.bold ? colors.bold(filename) : filename}\n`)
...
async function runSingleStepMode(
  ...
  const filename = file.split('/').pop() ?? file
  process.stdout.write(`\n`)
  process.stdout.write(`Step Test: ${colors.bold ? colors.bold(stepName) : stepName}  (${filename})\n`)

  if (tokenWarning) {
    process.stdout.write(`  Estimated tokens: ~${estimatedTokens}  ${colors.yellow(tokenWarning)}\n`)
  } else {
  ...
  if (stepContracts.length > 0) {
    ...
    for (const check of stepContracts) {
      ...
      if (check.status === 'pass') {
        ...
        process.stdout.write(`    ${edgeLabel.padEnd(40)} ${colors.dim('SKIP')} (${check.details})\n`)
      } else {
        process.stdout.write(`    ${edgeLabel.padEnd(40)} ${colors.red('FAIL')} ${check.details}\n`)
      }
    }
  } else {
  ...
  if (branchEvals && branchEvals.length > 0) {
    ...
    for (const branch of branchEvals) {
      ...
      if (branch.error) {
        process.stdout.write(`    ${label.padEnd(30)} ${colors.yellow(`error: ${branch.error}`)}\n`)
      } else if (typeof branch.result === 'boolean') {
        const resultStr = branch.result ? colors.green('true') : colors.red('false')
        process.stdout.write(`    ${label.padEnd(30)} {{ ${branch.expression} }} -> ${resultStr}\n`)
      } else {
        process.stdout.write(`    ${label.padEnd(30)} ${branch.expression} -> ${String(branch.result)}\n`)
      }
    }
  }
