...
function diffSpecs(a: LogicSpec, b: LogicSpec): DiffSection[] {
	...
	if (
		stepsSection.added.length > 0 ||
		stepsSection.removed.length > 0 ||
		stepsSection.changed.length > 0
	) {
		sections.push(stepsSection);
	}

	// d. Contract Inputs
	const inputsSection: DiffSection = {
		label: "Contract Inputs",
		added: [],
		removed: [],
		changed: [],
	};
	...
	if (
		inputsSection.added.length > 0 ||
		inputsSection.removed.length > 0 ||
		inputsSection.changed.length > 0
	) {
		sections.push(inputsSection);
	}

	// e. Contract Outputs
	const outputsSection: DiffSection = {
		label: "Contract Outputs",
		added: [],
		removed: [],
		changed: [],
	};
	...
	if (
		outputsSection.added.length > 0 ||
		outputsSection.removed.length > 0 ||
		outputsSection.changed.length > 0
	) {
		sections.push(outputsSection);
	}

	// f. Quality Gates — Pre-Output
	const preOutputSection: DiffSection = {
		label: "Quality Gates (Pre-Output)",
		added: [],
		removed: [],
		changed: [],
	};
	...
	if (
		preOutputSection.added.length > 0 ||
		preOutputSection.removed.length > 0 ||
		preOutputSection.changed.length > 0
	) {
		sections.push(preOutputSection);
	}

	// g. Quality Gates — Post-Output
	const postOutputSection: DiffSection = {
		label: "Quality Gates (Post-Output)",
		added: [],
		removed: [],
		changed: [],
	};
	...
	if (
		postOutputSection.added.length > 0 ||
		postOutputSection.removed.length > 0 ||
		postOutputSection.changed.length > 0
	) {
		sections.push(postOutputSection);
	}

	// h. Quality Gates — Invariants
	const invariantsSection: DiffSection = {
		label: "Quality Gates (Invariants)",
		added: [],
		removed: [],
		changed: [],
	};
	...
	if (
		invariantsSection.added.length > 0 ||
		invariantsSection.removed.length > 0 ||
		invariantsSection.changed.length > 0
	) {
		sections.push(invariantsSection);
	}

	// i. Decision Trees
	const decisionTreesSection: DiffSection = {
	...
	if (
		decisionTreesSection.added.length > 0 ||
		decisionTreesSection.removed.length > 0 ||
		decisionTreesSection.changed.length > 0
	) {
		...
		.action((file1: string, file2: string) => {
			...
			if (!result1.ok) {
				...
				for (const err of result1.errors) {
					process.stderr.write(`${formatError(file1, err as ValidationError, colors)}\n`);
				}
				process.exit(1);
			}
			...
			if (!result2.ok) {
				...
				for (const err of result2.errors) {
					process.stderr.write(`${formatError(file2, err as ValidationError, colors)}\n`);
				}
				process.exit(1);
			}
