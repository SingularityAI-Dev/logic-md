...
import { parse, validate, resolveImports } from '@logic-md/core'
...
function applySpecVersionFix(filePath: string, content: string): string | null {
  ...
  const specVersionIdx = frontmatterLines.findIndex((l) =>
    l.startsWith('spec_version:'),
  )
...
function processFile(
  ...
  if (options.fix && initialErrors.length > 0) {
    const hasSpecVersionError = initialErrors.some(
      (e) =>
        e.path === '/spec_version' ||
        e.message.toLowerCase().includes('spec_version'),
    )
    ...
    .action(async (patterns: string[], options: ValidateOptions) => {
      ...
      if (options.json) {
        ...
        process.stderr.write(
          `${files.length} file(s) checked, ${errorCount} with errors\n`,
        )
